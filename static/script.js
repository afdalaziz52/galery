    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            result.data.forEach(filename => {
                ...
                imgDiv.innerHTML = `
                    <img src="/uploads/${filename}" alt="${filename}" class="w-full h-48 object-cover rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300" onclick="openModal('/uploads/${filename}')">
